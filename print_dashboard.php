...
<div class="page">
    ...
    <div class="report-header">
        ...
        <div class="report-title">
            ...
            <h2>Academic Report (All Semesters)</h2>
            <?php // DEBUG: Check batch_no value ?>
            ...
            <?php else: ?>
            <p style="font-size:12px;color:#ff0000;margin-top:6px;text-align:right;">Batch: [EMPTY - value: <?= isset($u['batch_no']) ? 'isset but empty' : 'not set' ?>]</p>
            <?php endif; ?>
    ...
    ?>
    <?php if ($bestSem || $worstSem): ?>
    <div class="section-title">Performance Highlights</div>
    <div class="summary-row">
        <?php if ($bestSem): ?>
        <div class="summary-card" style="background:#eafaf1;border-color:#27ae60;">
            <label style="color:#27ae60;">🏆 Best Semester</label>
            <span>Semester <?= $bestSem['sem'] ?></span>
            <div style="font-size:12px;color:#666;margin-top:4px;">SGI: <?= round($bestSem['sgi'], 2) ?> — <?= grade($bestSem['sgi']) ?></div>
        </div>
        <?php endif; ?>
        <?php if ($worstSem): ?>
        <div class="summary-card" style="background:#fef9f0;border-color:#f5a623;">
            <label style="color:#f5a623;">📉 Needs Improvement</label>
            <span>Semester <?= $worstSem['sem'] ?></span>
            <div style="font-size:12px;color:#666;margin-top:4px;">SGI: <?= round($worstSem['sgi'], 2) ?> — <?= grade($worstSem['sgi']) ?></div>
        </div>
        <?php endif; ?>
    </div>
    <?php endif; ?>
    ...
    <div class="declaration">
        <p>I hereby declare that all the information provided in this report including GPA, CGPA, and SGI scores are true and correct to the best of my knowledge.</p>
        <div class="sign-row">
